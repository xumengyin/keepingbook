...
import cpsdna.myapplication.keepingbook.util.Utils
...
class ScheduleTimeHeadersDecoration(
    ...
    private val dayFormatter = Utils.monthDayFotmat
    private val yearFormatter = Utils.yearFotmat
    ...
    private val timeSlots: Map<Int, StaticLayout> =sessions.mapIndexed { index, subPay ->
        index to createHeader(subPay.time)
    }.toMap()
...
//                        |RecyclerView.State: $state""".trimMargin()
 //               )
     ...
     */
    private fun createHeader(startTime: Long): StaticLayout {
        //取代simpleDateFormat
        ...
        val text =
            ...
            inSpans(AbsoluteSizeSpan(meridiemTextSize), StyleSpan(BOLD)) {
                append(yearFormatter.format(date).toUpperCase())
            }
        }
